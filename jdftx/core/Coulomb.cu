...
#define DECLARE_coulombAnalytic_gpu(Type) \
	...
	{	GpuLaunchConfigHalf3D glc(coulombAnalytic_kernel<Coulomb##Type##_calc>, S); \
		for(int zBlock=0; zBlock<glc.zBlockMax; zBlock++) \
			JDFTX_LAUNCH_T(coulombAnalytic_kernel, (Coulomb##Type##_calc), glc, zBlock, S, GGT, calc, data); \
	}
...
#define DECLARE_coulombAnalyticStress_gpu(Type) \
	...
	{	\
		...
		for(int zBlock=0; zBlock<glc.zBlockMax; zBlock++) \
			JDFTX_LAUNCH_T(coulombAnalyticStress_kernel, (Coulomb##Type##_calc), glc, zBlock, S, GGT, calc, X, Y, grad_RRT); \
	}
	...
	const complex* X, const complex* Y, symmetricMatrix3<>* grad_RRT)
{
	...
	for(int zBlock=0; zBlock<glc.zBlockMax; zBlock++)
		JDFTX_LAUNCH(coulombNumericalStress_kernel, glc, zBlock, S, GGT, Vc_RRT, X, Y, grad_RRT);
}
...
#define DECLARE_exchangeAnalytic_gpu(Type) \
	...
	{	\
		...
		for(int zBlock=0; zBlock<glc.zBlockMax; zBlock++) \
			JDFTX_LAUNCH_T(exchangeAnalytic_kernel, (Exchange##Type##_calc), glc, zBlock, S, GGT, calc, \
...
#define DECLARE_exchangeAnalyticStress_gpu(Type) \
	...
	{	\
		...
		for(int zBlock=0; zBlock<glc.zBlockMax; zBlock++) \
			JDFTX_LAUNCH_T(exchangeAnalyticStress_kernel, (Exchange##Type##_calc), glc, zBlock, S, G, calc, \
...
{	GpuLaunchConfig3D glc(multRealKernel_kernel, S);
	for(int zBlock=0; zBlock<glc.zBlockMax; zBlock++)
		JDFTX_LAUNCH(multRealKernel_kernel, glc, zBlock, S, kernel, data);
}
...
{	GpuLaunchConfig3D glc(multTransformedKernel_kernel, S);
	for(int zBlock=0; zBlock<glc.zBlockMax; zBlock++)
		JDFTX_LAUNCH(multTransformedKernel_kernel, glc, zBlock, S, kernel, data, offset);
}
...
{	GpuLaunchConfig3D glc(realKernelStress_kernel, S);
	for(int zBlock=0; zBlock<glc.zBlockMax; zBlock++)
		JDFTX_LAUNCH(realKernelStress_kernel, glc, zBlock, S, kernel_RRT, X, grad_RRT);
}
...
{	GpuLaunchConfig3D glc(transformedKernelStress_kernel, S);
	for(int zBlock=0; zBlock<glc.zBlockMax; zBlock++)
		JDFTX_LAUNCH(transformedKernelStress_kernel, glc, zBlock, S, kernel_RRT, X, grad_RRT, offset);
}